...
- **/two-axis-review** — review the branch against Standards and Spec with labeled findings, each carrying a validated fix proposal; mechanical validated fixes are auto-applied by a serial fix subagent (one revertable commit per finding), and you triage the rest in one reply (`STD-1 revert, SPEC-2 ticket, SPEC-3 later, …`), spinning session-sized fixes back into the ticket workflow and parking repo-wide patterns as standalone cleanup tickets, then open the PR that closes the spec issue
