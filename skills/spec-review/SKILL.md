...
3. **Route** by the contract's escalation test — **the defect's kind never decides this**; the list above says what to hunt. Derivable here, and so auto-applied: index drift (the decision unit's own text is the proof), a stale link, a stale-ground-truth correction *whose claim no decision's verdict leans on* — the code is the proof of what exists, but a decision that reasoned from the stale claim may need re-deciding, and that escalates — and any defect the spec's own record settles outright. The common case of the last is a body paragraph contradicting the decision that owns its question: the decision rules, the paragraph is a restatement that drifted, and correcting it is arithmetic rather than a verdict. Everything that adds, strikes, or picks among readings of a decision **escalates, with its drafted repair attached**, so the ask is a verdict on concrete text: underdetermined decisions, seam gaps, unfalsifiable acceptance, uncovered stories, orphans, scope leaks, and the contradictions whose halves both genuinely stand.
