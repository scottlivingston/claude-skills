...
Read the spec in full (body + comments), including its Testing Decisions and **Seams under test** list. List its sub-issue tickets with their blocking edges and states. Compute the wave structure (what's frontier now, what unblocks when) and report it.

### 2. Author and launch the workflow

Run the whole implementation as **one dynamic `Workflow`** (this skill is your authorization to use it). Pass the DAG — ticket ids, titles, blocking edges — as `args`. The script loops waves until no tickets remain open; per wave:
...
2. **Parallel implement**: one fresh agent per frontier ticket, each in an **isolated worktree**. Each agent gets the spec (Testing Decisions and Seams under test verbatim), its ticket body, and the `/implement` + `/tdd` discipline: red–green at the spec's seams, typecheck regularly, run single test files regularly, commit in its worktree. Its first action is confirming its ticket carries `in-progress`. Fresh context per ticket is the point — never two tickets in one agent. An agent that hits a decision the spec doesn't hold, or work no listed seam covers, **parks**: it unclaims the ticket, comments exactly what's missing on the ticket, and comments the gap on the **spec issue** so the spec stays truthful.
...
4. **Wave review**: one review pass over the wave's **combined merged diff** (split by subsystem if the wave is wide), along both axes — the repo's documented standards (sourced per the `/conventions` skill: `CONVENTIONS.md` at root plus any nearer scoped one, with the fallback sources for repos not using the convention), and fidelity to the tickets and spec (including the spec's inline decision snippets). Reviewing the integrated wave rather than each ticket alone is deliberate: it sees how parallel tickets compose.
