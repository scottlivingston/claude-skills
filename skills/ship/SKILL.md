...
3. **Per-ticket review**: review each finished ticket's diff along the two axes (standards + ticket/spec fidelity). A fix agent applies **only the mechanical tier** in the same worktree: documented-standard hard violations with a fix the reviewer states confidently — no proposer/validator pipeline mid-run. A finding where the code diverges from the spec because the *spec* may be wrong is ship's existing spec-gap case: comment it on the spec issue, don't "fix" it. Findings that look like instances of a repo-wide pattern (seen outside this ticket's diff) are collected for the hand-off, not fixed — partial fixes of a pattern make the repo less consistent.
...
Run the full test suite once more. Report per ticket: commits, review findings fixed vs deferred, parked tickets and why. If the per-ticket reviews collected repo-wide patterns, **offer** them as standalone cleanup tickets (`review-finding` + `needs-triage`, not children of the spec) — create them only if the user says yes; a declined offer will simply resurface in the follow-up `/two-axis-review`. Then point the user at the next step: `/two-axis-review <base>` with the spec issue as the Spec source — and, once that passes, a PR whose body `Closes #<spec>` (the sub-issue tickets are already closed with commit links, so the PR ties the branch to the whole tree).
