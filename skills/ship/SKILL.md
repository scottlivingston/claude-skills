...
4. **Wave review**: one review pass over the wave's **combined merged diff** (split by subsystem if the wave is wide), along both axes — the repo's documented standards, and fidelity to the tickets and spec (including the spec's inline decision snippets). Reviewing the integrated wave rather than each ticket alone is deliberate: it sees how parallel tickets compose.
