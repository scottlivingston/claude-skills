...
- **No bare internal IDs.** A decision ID, ticket number, or section name appears only *after* a clause saying what it is: "the decision that every dialog returns keyboard focus to its opener on close (D6)". The ID is for the record; the clause is for the reader. Same for locations — "the five admin-side dialogs" beats a crate path.
