...
Every ticket is either **HITL** — human in the loop, worked *with* a human who speaks for themselves — or **AFK**, driven by the agent alone. The mode is recorded as a `hitl` or `afk` label at creation (research is always `afk`; prototype, grilling, and design always `hitl`; task is decided per ticket). A HITL ticket only resolves through that live exchange; the agent never stands in for the human's side of it (a grilling agent that answers its own questions has broken this). Every HITL exchange speaks the project's **domain language** — capabilities and concepts, per `CONTEXT.md` — so the human engages with the decision, not with file paths. When a question lands on terrain they don't hold a model of, `/domain-expansion` briefs them in plain language on what the system does today; the code itself is one ask away, cited per `/code-anchors`.
...
4. `/to-tickets <spec>` — HITL: break the spec into sub-issue tickets; the quiz is where the human approves the breakdown.
5. `/ship <spec>` — implement the ticket DAG in parallel, fresh agent per ticket.
6. `/tareview` the branch against the spec, then merge via a PR that closes the spec issue.
