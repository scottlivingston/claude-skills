...
4. **The gate.** Escalations post as `<!-- map-review pending-questions -->` on the map (the workflow's final stage), notify, and end the turn per the contract. The question loop — here or in a later session — walks the tensions one per turn in the wayfinder HITL style: the project's domain language, tickets referred to by name, what each side says, why they can't both stand, the drafted repair, and a recommendation. Verdicts act as:
