...
3. **Route.** Auto-apply is the document bar — **provably derivable only**: index drift (the ticket's own text is the proof), a dead link, a superseded call whose later resolution *explicitly* revisits and settles the same question. A misfiling is derivable only when no ticket in the slice cites the decision at all; otherwise it is a tension. Everything else — contradictions, route gaps, undecided seams, scope leaks, supersessions that require picking a winner — is a **tension**, and tensions escalate; the pipeline never adjudicates one alone.
