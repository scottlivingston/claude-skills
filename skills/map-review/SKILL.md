...
- **Map mode**: the map must be complete — no open child tickets, **Not yet specified** empty, every closed deciding ticket filed (evidence tickets are never filed, per the wayfinder skill), every slice sealed. Anything still open means charting isn't done — stop, say so, and point at `/next`.
