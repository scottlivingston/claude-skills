...
2. **Read the terrain, then open per the contract.** Existing modules, interfaces, and conventions near the change; check `CONTEXT.md` for vocabulary and ADRs for constraints (per `/domain-modeling`). What you found *is* the context `/hitl-questions`' opening sets: lay it out, then ask the human how they picture the structure and reflect the split back before proposing anything of your own. Every structural question thereafter reaches them per that same contract — in the project's domain language, as what exists and how it behaves.
