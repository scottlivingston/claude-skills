...
A **closed map** isn't a dead end: `/specify` comments each spec's link on the map (and on its slice) before the map closes. Follow those links and route on the specs. Likewise a spec's stage depends on its `impl` sub-issues, fetched fresh each invocation.
...
- **The conductor conducts — stage substance never enters its context.** For the whole run it holds this skill, `/finding-pipeline`, `/hitl-questions`, and `/issue-tracker`; everything heavier is delegated:
